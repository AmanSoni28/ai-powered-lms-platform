...
const CreateCourse = () => {
    ...
    setLoading(true)
    try {
        const result = await axios.post(`${serverUrl}/api/course/create-course`, {title,category}, {withCredentials:true})
        const data = result.data
        // console.log(data);
        setLoading(false)
        toast.success(data.message)
        dispatch(setCreatorCoursesData([...creatorCoursesData, data.course]))
        navigate('/courses')
    ...
    <div className='min-h-screen flex items-center justify-center bg-gray-100 px-4 py-10'>
      <div className='max-w-xl w-[600px] mx-auto p-6 bg-white shadow-md rounded-md mt-10 relative'>
        ...
        <form onSubmit={handleCreateCourse} className="space-y-5">
          ...
          <button className="w-full bg-black text-white py-3 px-4 rounded-md active:bg-[#3a3a3a] hover:scale-105 hover:shadow-xl transition-all duration-200 active:scale-95 " disabled={loading}>{loading ? <ClipLoader size={30} color="white" /> : "Create"}</button>
        </form>
      </div>  
    </div>
  )
